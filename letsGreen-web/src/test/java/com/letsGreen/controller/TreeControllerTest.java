...
class TreeControllerTest {
    ...
    void testGetAllTrees() {
        List<Tree> treeList = Arrays.asList(tree);
    ...
    @Test
    void testUpdateTree() {
        when(treeService.updateTree(anyLong(), any(Tree.class))).thenReturn(tree);
    ...
    void testUpdateTreeNotFound() {
        when(treeService.updateTree(anyLong(), any(Tree.class))).thenReturn(null);
    ...
    void testDeleteTree() {
        ...
        assertEquals("Tree deleted", response.getBody());
        verify(treeService, times(1)).deleteTree(anyLong());
    }
}
